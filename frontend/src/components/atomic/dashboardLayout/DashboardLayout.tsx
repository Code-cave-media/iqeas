...
const menuConfig: Record<string, MenuItem[]> = {
  ...
  // You may also have estimation role here (optional)
  estimation: [{ label: "Dashboard", to: "/estimation", icon: Blocks }],
};
...
const DashboardLayout = ({ children }: { children: React.ReactNode }) => {
  ...
  // Who can switch accounts at all?
  const canSwitch = ["admin", "rfq", "project_coordinator"].includes(role);

  // Filter logic for which users should be shown
  const shouldShowUser = (u: any): boolean => {
    const targetRole = u.role?.toLowerCase();

    if (role === "admin") return true;
    if (role === "rfq") return targetRole === "estimation";
    if (role === "project_coordinator") return targetRole === "pm";
  ...
  const fetchUsers = async (page: number) => {
    ...
    try {
      ...
      toast.error("Failed to load switchable users");
    } finally {
  ...
  const filteredUsers = users.filter(shouldShowUser);
      ...
      >
        <div className="flex flex-col h-full">
          ...
          <div className="p-4 mb-2 flex flex-col items-center group-hover:items-stretch transition-all duration-300">
            ...
            {/* Switch Account - only for allowed roles */}
            {canSwitch && (
              <div
              ...
              >
                <button
                  ...
                  className="w-full flex items-center justify-between text-[11px] font-semibold tracking-wide uppercase text-slate-500 bg-slate-50 border border-slate-200/60 px-3 py-2 rounded-lg hover:bg-slate-100 hover:text-slate-700 transition-all disabled:opacity-50"
                    ...
                    {totalPages > 1 && (
                      <div className="p-2 bg-slate-50 flex items-center justify-between border-t border-slate-100">
                        <button
                          ...
                          className="p-1.5 rounded-md hover:bg-white border border-transparent hover:border-slate-200 disabled:opacity-40 transition"
                        >
                          <ChevronLeft size={14} />
                        </button>
                        ...
                        <button
                          disabled={
                            currentPage === totalPages || isLoadingUsers
                          }
                          onClick={() => fetchUsers(currentPage + 1)}
                          className="p-1.5 rounded-md hover:bg-white border border-transparent hover:border-slate-200 disabled:opacity-40 transition"
                        >
