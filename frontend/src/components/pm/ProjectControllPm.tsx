...
const ProjectControlPm: React.FC = () => {
  ...
  // stage manager
  const [stageOptions, setStageOptions] = useState<string[]>(["IDC", "IFR", "IFA", "IFC"]);
  const [showStageModal, setShowStageModal] = useState(false);
  const [stageInput, setStageInput] = useState("");
  const [stageInputList, setStageInputList] = useState<string[]>([]);
  ...
  const revisionOptions = useMemo(() => {
    const set = new Set<string>();
    rfqDeliverables.forEach((d) => {
      if (d.revision !== undefined && d.revision !== null && d.revision !== "") {
        set.add(String(d.revision));
      }
    });
    return Array.from(set).sort();
  ...
  const renderProjectTab = () => (
    <div className="rounded-2xl shadow-lg border bg-white">
      ...
      <div className="px-6 py-4 flex items-center gap-3">
        <Progress
          value={project.status === "draft" ? 50 : Number(project.progress) || 100}
          className="h-2 bg-gray-200 flex-1"
        />
        <span className="text-xs font-mono text-slate-600">
            ...
            <table className="w-full text-sm border rounded-lg overflow-hidden">
              ...
              <tbody>
                {visibleRows.map((item, index) => {
                  ...
                  const lastChildStage =
                    isParent && parentHasChild ? getLastChildStage(item) : undefined;
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Stage: independent select + dummy label for parents */}
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <select
                            value={item.stage ?? ""}
                          ...
                          {isParent && parentHasChild && lastChildStage && (
                            <span className="text-[10px] text-slate-500">
                              Last sub: {lastChildStage}
                            </span>
                          )}
                        </div>
                      </td>
  ...
  return (
    <div className="w-full h-full flex flex-col bg-slate-50">
      ...
      {/* Delivery dialog (reuse your existing UI if needed) */}
      <Dialog open={showDeliveryDialog} onOpenChange={setShowDeliveryDialog}>
        <DialogContent className="p-6 max-w-lg">
          <DialogHeader>
            <DialogTitle>Select Delivery Files</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            {files.length === 0 ? (
